...
class Rollout(object):
    def __init__(self, generator, update_rate, decoder_scope):
        ...
        with tf.variable_scope(decoder_scope, reuse=True):
            ...
            def recurrence_rollout(i, dec_input):
                ...
                output_id = self.generator.decode_onestep(dec_input_emb, sample_fn=tf.nn.top_k)
            ...
            _, self.rollout_sample_ar = control_flow_ops.while_loop(
                cond=lambda i, _1: i < max_dec_steps,
    ...
    def get_reward(self, hps_gan, sess, dec_vocab, source_batch, enc_states, k_samples, k_targets_ns, discriminator):
        ...
        k_rewards = []
        weights = np.transpose(np.tile(np.array(map(lambda i: 0.8**i, range(max_dec_steps))), (batch_size, 1)))
        ...
        for k, samples in enumerate(k_samples):
            ...
            for given_num in range(hps_gan.rollout_start, max_dec_steps):
                ...
                if rouge_ratio:
                    rouge_scores = []
                    summaries = outputsids2words(strip_pads(rollout_samples.tolist(), dec_stop_id), dec_vocab)
            ...
            if dis_ratio:
                emb_targets = sess.run(
                    self.generator.dec_temp_embedded,
                    feed_dict={self.generator.dec_temp_batch: k_targets_ns[k]})

                # the last token reward
                feed = {
                    discriminator.inputs: emb_targets,
                    discriminator.conditions: emb_articles,
                    discriminator.condition_lens: article_lens
                }
                ypred_for_auc = sess.run(discriminator.dis_ypred_for_auc, feed)
                ...
                if hps_gan.subtract:
                    dis_rewards = dis_rewards - dis_rewards[-1]

            if rouge_ratio:
                rouge_scores = []
                summaries = outputsids2words(strip_pads(samples.tolist(), dec_stop_id), dec_vocab)
                references = source_batch.original_abstracts
                for s, r in zip(summaries, references):
                    rouge = rouge_l(s, r.split())
                    rouge_scores.append(rouge)
                ...
                if hps_gan.subtract:
                    rouge_rewards = rouge_rewards - rouge_rewards[-1]
            ...
            decayed_rewards = rewards * weights
            k_rewards.append(decayed_rewards)

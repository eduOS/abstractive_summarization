...
def average_clip_gradient_by_value(grad_and_vars, clip_min, clip_max):
    grads = []
    for g, _ in grad_and_vars:
        # Add 0 dimension to the gradients to represent the tower.
        expanded_g = tf.expand_dims(g, 0)
    ...
    grad_and_var = (grad, v)
    if clip_max > 0:
        grad, value = grad_and_var
        grad = tf.clip_by_value(grad, clip_min, clip_max)
        average_grad = grad, value

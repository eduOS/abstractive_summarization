...
def highway_s(input_, size, layer_size=1, bias=-2, f=tf.nn.relu, reuse_var=False):
    ...
    if reuse_var == True:
        tf.get_variable_scope().reuse_variables()
    for idx in xrange(layer_size):
        output = f(tf.nn.rnn_cell._linear(output, size, 0, scope='output_s_lin_%d' %idx))
        transform_gate = tf.sigmoid(tf.nn.rnn_cell._linear(input_, size, 0, scope='transform_s_lin_%d'%idx) +bias)
        carry_gate = 1. - transform_gate
        output = transform_gate * output + carry_gate * input_
    return output
...
class DisCNN(object):
  ...
  def __init__(self, sess, max_len, num_classes, vocab_size, batch_size,  dim_word, filter_sizes, num_filters, source_dict, target_dict, gpu_device, positive_data, negative_data, source_data,
       ...
       ## build_model ##########
        ...
        params = [param for param in tf.all_variables() if self.scope in param.name]
  ...
  def build_model(self, reuse_var=False, gpu_device='0'):
     with tf.variable_scope(self.scope):
        with tf.device('/gpu:%d' % int(gpu_device)):
            ...
            losses = tf.nn.softmax_cross_entropy_with_logits(scores, input_y_trans)

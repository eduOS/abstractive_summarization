...
class Rollout(object):
    def __init__(self, generator, update_rate):
        ...
        with tf.variable_scope('rollout_loops'):
            def recurrence_given(i, enc_states, dec_input, dec_in_state, given_num, gen_summ):
                next_input_id, new_state = self.generator.decode_onestep(enc_states, [dec_input], dec_in_state)
                next_input = emb_summ_ar.read(i)
                gen_summ = gen_summ.write(i, summ_ar.read(i))
                return i+1, enc_states, next_input, new_state, given_num, gen_summ

            def recurrence_rollout(i, enc_states, dec_input, dec_in_state, given_num, gen_summ):
                next_input_id, new_state = self.generator.decode_onestep(enc_states, [dec_input], dec_in_state)
            ...
            i, _, next_input, new_state, given_num, self.gen_summ_ar = control_flow_ops.while_loop(
                cond=lambda i, _0, _1, _2, given_num, _4: i < given_num,
                body=recurrence_given,
                loop_vars=(tf.constant(1, dtype=tf.int32), self.enc_states, emb_summ_ar.read(0),
            ...
            _, _, _, _, _, self.gen_summ_ar = control_flow_ops.while_loop(
                cond=lambda i, _0, _1, _2, _3, _4: i < self._gen_hps.max_dec_steps,
                body=recurrence_rollout,
                loop_vars=(i, self.enc_states, next_input, new_state, given_num, self.gen_summ_ar))
    ...
    def get_reward(self, sess, gen_vocab, dis_vocab, source_batch,
        ...
        for i in range(rollout_num):
            for given_num in range(1, self._gen_hps.max_dec_steps):
                ...
                feed_dict[self.enc_states] = enc_states
